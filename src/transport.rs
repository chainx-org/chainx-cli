...
pub type BoxFuture<'a, T> = Box<dyn Future<Item = T, Error = Error> + 'a>;
...
impl<T: BatchTransport> ChainXTransport<T> {
    ...
    pub fn execute(&self, method: &str, params: Vec<Value>) -> BoxFuture<Value> {
        Box::new(self.transport.execute(method, params).map_err(Into::into))
    }
}
